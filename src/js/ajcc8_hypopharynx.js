...
`;
    ...
    if ($('.cb_rn:checked').length) {
        if ($('#cb_rn_ene').is(':checked')) {
            ...
            n_stage.push("3a");
                    ...
                    || ($('.cb_tl_r:checked').length && $('.cb_rn_l:checked').length)
                    || ($('.cb_tl_l:checked').length && $('.cb_rn_r:checked').length)   ) {
            n_stage.push("2c");
    ...
    let t_str = AJCC8_HYPOPHARYNX_T[t];
    let n_str = AJCC8_HYPOPHARYNX_N[n];
    let m_str = AJCC8_HYPOPHARYNX_M[m];
    report += ajcc_template("Hypopharynx Carcinoma", t, t_str, n, n_str, m, m_str);
