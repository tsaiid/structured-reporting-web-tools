...
if (process.env.NODE_ENV !== 'production') {
    require('raw-loader!../html/ajcc8/oral.html');
}

import {join_checkbox_values, ajcc_template} from './ajcc8_common.js';

const AJCC8_ORAL_T_LIP = {
    ...
    '4a': 'Moderately advanced local disease: Tumor invades through cortical bone or involves the inferior alveolar nerve, floor of mouth, or skin of face (i.e., chin or nose).',
...
const AJCC8_ORAL_T_ORAL = {
    ...
    '2': 'Tumor > 2 cm but ≤ 4 cm in greatest dimension.',
    '3': 'Tumor > 4 cm in greatest dimension.',
    '4a': 'Moderately advanced local disease: Tumor invades adjacent structures only (e.g., through cortical bone [mandible or maxilla] into deep [extrinsic] muscle of tongue [genioglossus, hyoglossus, palatoglossus, and styloglossus], maxillary sinus, skin of face).',
    '4b': 'Very advanced local disease: Tumor invades lateral pterygoid muscle, pterygoid plates, lateral nasopharynx, or skull base or encases carotid artery.',
};
const AJCC8_ORAL_N = {
...
`;
    ...
    if ($('#cb_tl_ul, #cb_tl_ll').is(':checked')) {
        if ($('.cb_ti:checked').length) {
            report += "--- Yes:\n";
            report += "* " + join_checkbox_values($('.cb_ti_lip:checked'), "\n* ");
            report += "\n";

            if ($('.cb_ti_lip.cb_ti_t4a:checked').length) {
                t_stage.push("4a");
            }
        ...
        if ($('.cb_ti:not(:checked)').length) {
            ...
            var ti_array = [];
            if ($('.cb_ti_lip.cb_ti_t4a:not(:checked)').length) {
                ti_array.push("* " + join_checkbox_values($('.cb_ti_lip.cb_ti_t4a:not(:checked)')));
            }
        ...
        if ($('.cb_ti:checked').length) {
            report += "--- Yes:\n";
            report += "* " + join_checkbox_values($('.cb_ti_oral:checked'), "\n* ");
            report += "\n";

            if ($('.cb_ti_oral.cb_ti_t4a:checked').length) {
                t_stage.push("4a");
            }
            if ($('.cb_ti_t4b:checked').length) {
                t_stage.push("4b");
        ...
        if ($('.cb_ti:not(:checked)').length) {
            report += "--- No or Equivocal:\n";
            var ti_array = [];
            if ($('.cb_ti_oral.cb_ti_t4a:not(:checked)').length) {
                ti_array.push("* " + join_checkbox_values($('.cb_ti_oral.cb_ti_t4a:not(:checked)')));
            }
            if ($('.cb_ti_t4b:not(:checked)').length) {
                ti_array.push("* " + join_checkbox_values($('.cb_ti_t4b:not(:checked)')));
            }
    ...
    let AJCC8_ORAL_T = ($('#cb_tl_ul, #cb_tl_ll').is(':checked') ? AJCC8_ORAL_T_LIP : AJCC8_ORAL_T_ORAL );
    let t_str = AJCC8_ORAL_T[t];
    let n_str = AJCC8_ORAL_N[n];
    let m_str = AJCC8_ORAL_M[m];
